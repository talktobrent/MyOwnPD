...
document.addEventListener("DOMContentLoaded", ()=> {
    ...
    var history = {
        ...
        phone: document.getElementById("phoney"),
...
//        },
        ...
        getZip: function(zip) {
            this._beforeZip(zip);
            if (this.state[zip]) {
                this._alreadyZip(zip);
                this._afterZip()
            }
            else {
                this.socket.send(JSON.stringify({"zip": zip}));
            }
        },

//        changeZip: async function(json) {
//
//            this.viewLocation(false);
//            this.loadingMap();
//            this.hideOptions(this.state.zip);
...
//        },
        ...
        optionsBuilder: async function(zip, locations) {
            ...
            for (let [id, loc] of Object.entries(locations)) {
                if (!this.state[id]) {
                    ...
                    this.state[id].hidden = false;
                    existing.push(this.state[id]);
                }
            }
            fragment.append(...options)
            this.hals.appendChild(fragment);
            this.state[zip].options = options.concat(existing);
        },
        ...
        insertCommit: function(location, commit) {
            let spot = location.querySelector("#commit");
            spot.textContent = spot.datetime = commit;
        },
        getCommit: function(option) {
            if (!option.commit) {
                this.socket.send(JSON.stringify({"commit": option.className, "id": option.id}));
            }
            else if (!option.committed) {
                this.insertCommit(option.location, option.commit);
                option.committed = true;
            }
        },
//        zipState: function() {
//            if (this.zip.checkValidity()) {
...
//        }
    ...
    history.socket.onmessage = function (event) {
        data = JSON.parse(event.data);
        if ('phone_valid' in data) {
            history.phone.value = data['phone_input']
            if (!data['phone_valid']) {
                history.phone.setCustomValidity('Enter valid phone number');
//                history.phone.reportValidity();
            }
            else {
                history.phone.setCustomValidity('');
//                history.phone.reportValidity();
            }
        }
            ...
            history.mapZoomer(data['coords'], true)
        }
        else if (data['commit']) {
            history.state[data['id']].commit = data['commit'];
            history.insertCommit(history.state[data['id']].location, data['commit']);
            history.hals.querySelectorAll("[class='" + data['zip'] + "']").forEach((option) => {
                option.commit = data['commit']
            });
        }
    };
...
//    document.querySelector('head').appendChild(script);
    ...
    history.phone.addEventListener("input", (e)=> {
        if (history.phone.value) {
            history.phone.value = history.phone.value.replace(/[^0-9]/g,'').slice(0, 10)
            history.socket.send(JSON.stringify({"phone": history.phone.value}));
        }
    });
    history.zip.addEventListener("input", (e)=> {
        if (history.zip.value) {
            history.zip.value = history.zip.value.replace(/[^0-9]/g,'');
        }
        if (history.zip.value.length === 5 && history.zip.value !== history.state.zip.zip) {
            history.getZip(history.zip.value);
            history.zip.removeAttribute('required');
        }
    });
    ...
    history.detail.addEventListener("click", (event) => {
        if (history.map.hidden && !history.detail.open) {
            ...
            history.zip.setCustomValidity('Please enter zip code')
            history.zip.reportValidity();
//            history.zip.setCustomValidity('');
            return;
        }
    ...
    history.payment.addEventListener("toggle", (event) => {
        if (!history.checkout) {
            striper();
            history.checkout = true;
        }
    });
